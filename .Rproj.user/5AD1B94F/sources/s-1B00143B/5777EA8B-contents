...
# The spread is not stationary, and does not seem to present a linear trend, it is probably integrated of order at least 1
# dspread looks stationary and the mean seems to be close to zero, it is probably integrated of order 0

# Q3 ####
# Before making unit root test we test that there is not a linear trend
summary(lm(spread ~ dates))
# the coefficient is negative and statistically significant, so we can reject the null hypothesis of no linear trend
...
# We are going to look at the adjusted R squared to choose the best model.
...
# We can fix it with a dummy variable
