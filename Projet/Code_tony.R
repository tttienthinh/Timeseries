...
for (p in 1:7) {
  arma_model <- arima(x, order = c(p, 1, 1), include.mean = FALSE)
...
# Test de portemanteau sur les résidus pour vérifier l'absence d'autocorrélation
residuals <- residuals(model)
Box.test(residuals, lag = 24, type = "Ljung-Box")
#on ne rejette pas l'hypothèse d'indépendance des résidus jusqu'au lag 24, ce qui est bon signe
